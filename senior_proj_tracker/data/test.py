...
theta_north = np.radians(10.89234794)  # north direction (rad)
...
# ===== ON ANALYSIS =====
theta_eq = np.mean(theta_on[-len(theta_on)//10:])

def simulate(m, th_n):
    def ode(t, y):
        th, thdot = y
        thddot = (-b * thdot - k * (th - theta0) + m * B * np.sin(th - th_n)) / I
        return [thdot, thddot]
...
def error(params):
    m, th_n = params
    try:
        th_sim = simulate(m, th_n)
        return np.sum((th_sim - theta_on)**2)
    except:
        return 1e10

# Grid search then refine
best_err, best_p = 1e10, None
for m_g in np.linspace(-0.5, 0.5, 11):
    for th_n_g in np.linspace(-np.pi, np.pi, 19):
        e = error([m_g, th_n_g])
        if e < best_err:
            best_err, best_p = e, [m_g, th_n_g]

res = minimize(error, best_p, method='Nelder-Mead', options={'maxiter': 2000})
m_ode, th_n_fit = res.x

# Steady state with fitted theta_north
m_ss = k * (theta_eq - theta0) / (B * np.sin(theta_eq - th_n_fit))
...
print(f"theta_north_fit = {np.degrees(th_n_fit):.4f} deg")

plt.figure()
plt.plot(t_on, np.degrees(theta_on), 'b.', markersize=2, label='Data')
plt.plot(t_on, np.degrees(simulate(m_ss, th_n_fit)), 'r-', label=f'Steady state m={m_ss:.4e}')
plt.plot(t_on, np.degrees(simulate(m_ode, th_n_fit)), 'g--', label=f'ODE fit m={m_ode:.4e}')
plt.xlabel('Time (s)')
plt.ylabel('θ (deg)')
